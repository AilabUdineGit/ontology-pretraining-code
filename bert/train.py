...
def main(train):
    ...
    model = AutoModelForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=len(label_to_int))
    ...
    def make_tokenize_function(max_len):
        if max_len is None:
            ...
            return lambda examples: tokenizer(examples["text"], padding="max_length", truncation=True, max_length=max_len)
            ...
            for samp in data:
                if samp["label"] in label_to_int:
                    self.input_ids.append(
                        torch.tensor(samp['input_ids'])
                    )
                    self.attn_masks.append(
                        torch.tensor(samp['attention_mask'])
                    )
                    self.labels.append(
                        torch.tensor(label_to_int[samp["label"]])
                    )
#                     print(samp["label"])
    ...
    training_args = TrainingArguments(
        ...
        gradient_accumulation_steps=GRADIENT_ACCUMULATION_STEPS if args.dataset != "llt_to_pt" else 4,
        # ------------------------------------------------------- [hyperparams]
        warmup_steps=100, 
        weight_decay=0.01,
        # ------------------------------------------------------- [save and logging]
