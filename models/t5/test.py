...
def main(args):
    ...
    if args.ft:
        checkpoint_input_path = os.path.join(args.test_load_path, args.dataset, f"run_{args.run}","model")
    else:
...
def report(generated,aes, terms,args):
    ...
    report = classification_report(out_df.gold_label, out_df.model_generated, zero_division=0, output_dict=True)
    A=report["accuracy"]
    B=report["macro avg"]
    ...
    if args.dataset_test and args.dataset_test != args.dataset:
        save_path =  os.path.join(args.results_save_path,f"{args.dataset}_to_{args.dataset_test}",f"run_{args.run}/")
    else:
