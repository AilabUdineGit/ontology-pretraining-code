...
class MeddraPredictor_unconstrained_multiple(nn.Module):
    ...
    def forward(self, test_set):
        ...
        with torch.no_grad():
            ...
            for samps in tqdm(batch(test_set, BATCH_SIZE), desc="predicting", total=len(test_set)//BATCH_SIZE):
                ...
                prompt = self.tokenizer(prompt_str, return_tensors="pt", padding=True).to(self.model.device)
                ...
                generated = [
                    ...
                    if 0 in prompt.attention_mask[idx] else
                    gen[:, prompt.attention_mask[idx].shape[0]:]
