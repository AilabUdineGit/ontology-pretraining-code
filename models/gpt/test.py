...
def main(train):
    ...
    parser.add_argument(
        ...
        "--test_dataset",
        default=None,
        help="name of the test dataset if different from the training dataset",
    )
    parser.add_argument(
    ...
    generator = dataset._generate_examples(
        f"../dataset/{args.dataset}/run_{SPLIT}/test.csv", "test"
    )
    test_samples = [
        dict(
    ...
    mymodel = MeddraPredictor_unconstrained_multiple(
        WEIGHTS_OUTPUT_DIR + "/model", BEFORE_SAMPLE, BEFORE_LABEL
    ).to(DEVICE)
